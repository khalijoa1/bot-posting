...
async def list_posts(message: types.Message):
    ...
    if not posts:
        await message.reply("You have no posts.", parse_mode=None)
        return
    ...
    for p in posts:
        ...
        lines.append(f"Post ID={p.id}: {preview}... ({ch_count} channels)")
...
async def process_post_id(message: types.Message, state: FSMContext):
    ...
    if not targets:
        await message.reply("This post has no active messages", parse_mode=None)
        await state.clear()
        return
    ...
    kb = types.InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [types.InlineKeyboardButton(text="🗑️ Delete", callback_data="action_delete")],
        ...
        async with session() as s:
            ...
            for target in targets:
                try:
                    ...
                    await query.message.reply(f"❌ Failed to delete from {target.channel.title}", parse_mode=None)

            await s.commit()

        await query.message.reply(f"✓ Deleted from {deleted_count} channel(s)", parse_mode=None)
        await state.clear()
        await query.answer()
        return

    if action == "edit":
        await query.message.reply("Send the new text:", parse_mode=None)
    ...
    async with session() as s:
        ...
        for target in targets:
            try:
                ...
                await message.reply(f"❌ Failed to edit in {target.channel.title}: {str(e)}", parse_mode=None)

        await s.commit()

    await message.reply(f"✓ Edited in {edited_count} channel(s)", parse_mode=None)
    await state.clear()
